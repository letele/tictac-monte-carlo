...
source("R/generate_random_board.R")

# function to print board
print_board <- function(board) {
  chars <- c("O", ".", "X")[board + 2]
  board_matrix <- matrix(chars, nrow = 3, byrow = TRUE)
  board_string <- apply(board_matrix, 1, paste, collapse = " ")
  cat(paste(board_string, collapse = "\n"), "\n")
}
...
plot_tree <- function(tree_data) {
  ...
  if (length(term_idx) > 0) {
    ...
    if (length(act_idx) > 0) {
      ...
      # Active points are slightly larger
      p_nodes$n_size[act_idx] <- 3
    }
...
```{r}
# set.seed(12)
tree_data <- monte_carlo_tree_search(b,1,1)
```
...
d <- mcts_df(mcts_stats(b,1,1))
d
```

```{r}
mcts_predict_cell(b,-1,d)
```


```{r}
s <- colSums(d[c("win", "lose", "draw")])
total <-  sum(s)
...
lapply(s[s > 0], function(x) simplify_fraction(x, total))
